...
// Sucursales con sus claves
const SUCURSALES_CLAVES = {
  "Villahermosa":               "vhsa2026",
  "Ciudad del Carmen":          "carmen2026",
  "Coatzacoalcos":              "coat2026",
  "Mérida":                     "mid2026",
  "Campeche":                   "camp2026",
  "Cancún":                     "cancun2026",
  "Playa del Carmen":           "playa2026",
  "Cozumel":                    "coz2026",
  "Chetumal":                   "chet2026",
  "Tuxtla Gutiérrez":           "tuxtla2026",
  "San Cristóbal de las Casas": "sancris2026",
  "Tapachula":                  "tapa2026",
  "Oaxaca":                     "oax2026",
  "Tuxtepec":                   "tux2026",
  "Salina Cruz":                "salina2026",
  "Puebla":                     "pue2026",
  "Tehuacán":                   "tehua2026",
  "Tlaxcala":                   "tlax2026",
  "Veracruz":                   "ver2026",
...
function PantallaLogin({ onLogin }) {
  ...
  const intentarLogin = () => {
    // Buscar en roles de dirección/regional
    for (const [, rol] of Object.entries(ROLES)) {
      if (clave === rol.clave) {
        onLogin({ tipo: rol.tipo, dir: rol.dir || null, label: rol.label });
        return;
      }
    }
    ...
    for (const [suc, claveSuc] of Object.entries(SUCURSALES_CLAVES)) {
      if (clave === claveSuc) {
        // Encontrar la dirección de esta sucursal
        for (const [dir, sucs] of Object.entries(ESTRUCTURA)) {
          if (sucs.includes(suc)) {
            onLogin({ tipo: "sucursal", suc, dir, label: suc });
            return;
          }
        }
      }
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#1e293b,#0f172a)",
      ...
      <div style={{ background: "#fff", borderRadius: 24, width: "100%", maxWidth: 380,
        ...
        <div style={{ background: "linear-gradient(135deg,#1e3a5f,#0f172a)", padding: "32px 28px", textAlign: "center" }}>
          <div style={{ width: 64, height: 64, borderRadius: 16, background: "rgba(255,255,255,0.1)",
        ...
        <div style={{ padding: 28 }}>
          <div style={{ marginBottom: 20 }}>
            ...
            {error && (
              <div style={{ fontSize: 12, color: "#dc2626", marginTop: 6 }}>
                ⚠️ Clave incorrecta. Intenta de nuevo.
              </div>
            )}
          </div>
          ...
          <div style={{ marginTop: 20, background: "#f8fafc", borderRadius: 12, padding: 14 }}>
            <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 600, textTransform: "uppercase", letterSpacing: .5, marginBottom: 8 }}>
              Accesos disponibles
            </div>
            <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.8 }}>
              🏢 Director Regional<br/>
              🗂️ Directores Estatales (4)<br/>
              🏬 Responsables de Sucursal (22)
            </div>
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  if (!usuario) return <PantallaLogin onLogin={u => { setUsuario(u); setConfigAbierta(u.tipo === "regional"); }} />;
  ...
  // Vista de sucursal — va directo a captura
        ...
        <div style={{ padding: 28, display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div>
            ...
            <input type="number" min="1" max="31" value={configForm.diasHabiles}
              ...
              style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: "1.5px solid #e2e8f0", fontSize: 16, fontFamily: "'DM Mono',monospace", outline: "none", boxSizing: "border-box" }} />
          </div>
          <div>
  ...
  // Vista directa para sucursal
  const VistaSucursal = () => {
    const { suc, dir } = usuario;
    ...
    const ed = editando || { dir, suc };

    return (
      <div style={{ maxWidth: 520, margin: "0 auto", padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
        {/* Header sucursal */}
        <div style={{ background: col, borderRadius: 16, padding: "18px 24px", color: "#fff" }}>
          <div style={{ fontSize: 10, opacity: .75, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase" }}>Mi sucursal · {mesLabel}</div>
          <div style={{ fontSize: 24, fontWeight: 700 }}>{suc}</div>
          <div style={{ fontSize: 12, opacity: .75 }}>Dirección {dir} · Día hábil {diaActual}/{diasHabiles}</div>
        ...
        <div style={{ background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 1px 6px rgba(0,0,0,0.07)", border: "1px solid #e2e8f0" }}>
          <div style={{ background: "#f8fafc", padding: "12px 20px", borderBottom: "1px solid #e2e8f0",
            ...
            {capturaHoy && <span style={{ background: "#dcfce7", color: "#16a34a", fontSize: 10, fontWeight: 700, borderRadius: 20, padding: "2px 10px" }}>✓ Guardado</span>}
          </div>
          <div style={{ padding: 20 }}>
            {capturaHoy ? (
              editDia?.dia === diaActual ? (
                <div>
                  <input type="number" min="0" value={editDia.monto}
                    ...
                    style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: `1.5px solid ${col}`, fontSize: 17, fontFamily: "'DM Mono'", outline: "none", boxSizing: "border-box" }} />
                  <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                    <button onClick={() => setEditDia(null)} style={{ flex: 1, padding: "10px", borderRadius: 10, border: "1.5px solid #e2e8f0", background: "#fff", color: "#64748b", fontFamily: "inherit", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>Cancelar</button>
                    <button onClick={async () => { await guardarEnSheets(dir, suc, diaActual, editDia.monto); setEditDia(null); }} disabled={guardando}
                      style={{ flex: 2, padding: "10px", borderRadius: 10, border: "none", background: col, color: "#fff", fontFamily: "inherit", fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => setEditDia({ dia: diaActual, monto: capturaHoy.monto })}
                      style={{ padding: "8px 14px", borderRadius: 8, border: `1.5px solid ${col}`, background: "#fff", color: col, fontFamily: "inherit", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>✏️ Editar</button>
                    <button onClick={async () => { await borrarEnSheets(dir, suc, diaActual); }} disabled={guardando}
                      style={{ padding: "8px 14px", borderRadius: 8, border: "1.5px solid #fca5a5", background: "#fff", color: "#dc2626", fontFamily: "inherit", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                      {guardando ? "..." : "🗑"}
                    </button>
                  </div>
                </div>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: "#64748b", display: "block", marginBottom: 8 }}>💰 Monto colocado hoy</label>
                <input type="number" min="0" value={form.montoHoy} onChange={e => setForm({ montoHoy: e.target.value })} placeholder="0"
                  style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: `1.5px solid ${col}`, fontSize: 18, fontFamily: "'DM Mono'", outline: "none", boxSizing: "border-box", color: "#0f172a" }} />
                {mHoy > 0 && <div style={{ background: "#f8fafc", borderRadius: 10, padding: 10, marginTop: 10, fontSize: 12, color: "#64748b" }}>Nuevo acumulado: <strong style={{ color: "#0f172a", fontFamily: "'DM Mono'" }}>{fmtMXN(acu + mHoy)}</strong></div>}
                <button onClick={async () => { if (mHoy > 0) { await guardarEnSheets(dir, suc, diaActual, form.montoHoy); setForm({ montoHoy: "" }); } }} disabled={!mHoy || guardando}
                  style={{ width: "100%", marginTop: 12, padding: "12px", borderRadius: 10, border: "none", background: mHoy > 0 ? col : "#e2e8f0", color: mHoy > 0 ? "#fff" : "#94a3b8", fontFamily: "inherit", fontWeight: 700, fontSize: 14, cursor: mHoy > 0 ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                  ...
                  <div key={d.dia} style={{ padding: "12px 20px", borderTop: i > 0 ? "1px solid #f1f5f9" : "none", background: esHoy ? "#f0fdf4" : "#fff" }}>
                    {enEd ? (
                      <div>
                        ...
                        <input type="number" min="0" value={editDia.monto} onChange={e => setEditDia(ed => ({ ...ed, monto: e.target.value }))}
                          style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: `1.5px solid ${col}`, fontSize: 16, fontFamily: "'DM Mono'", outline: "none", boxSizing: "border-box" }} />
    ...
    <div style={{ fontFamily: "'DM Sans',sans-serif", background: "#f1f5f9", minHeight: "100vh" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {esRegional && (
            <button onClick={() => { setConfigForm({ diasHabiles, diaActual, mes: mesActual, anio: anioActual }); setConfigAbierta(true); }}
              style={{ background: "rgba(255,255,255,0.12)", border: "none", borderRadius: 8, color: "#fff", cursor: "pointer", padding: "6px 12px", fontSize: 12, fontFamily: "inherit" }}>⚙️</button>
          )}
          <button onClick={() => setUsuario(null)}
            style={{ background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 8, color: "#fff", cursor: "pointer", padding: "6px 12px", fontSize: 12, fontFamily: "inherit" }}>Salir</button>
        </div>
      </header>
      ...
      {!esSucursal && (
        <main style={{ padding: 20, maxWidth: 1100, margin: "0 auto" }}>
              ...
              {Object.keys(ESTRUCTURA).map(dir => {
                ...
                return (
                  <div key={dir} onClick={() => { setDirSel(dir); setVista("detalle"); }}
                    ...
                    style={{ background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 1px 6px rgba(0,0,0,0.07)", border: "1px solid #e2e8f0", cursor: "pointer", transition: "box-shadow .2s", display: "flex", flexDirection: "column" }}>
          ...
          {/* ══ DETALLE DIRECCIÓN (solo regional) ══ */}
                      ...
                      {sucs.map((suc, i) => {
                        ...
                        return (
                          <tr key={suc} style={{ borderTop: "1px solid #f1f5f9", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                            ...
                            <td style={{ padding: "12px 14px" }}>
                              <button onClick={() => { setEditando({ dir: dirSel, suc }); setForm({ montoHoy: "" }); setEditDia(null); setVista("captura"); }}
            ...
            return (
              <div style={{ maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ background: col, borderRadius: 16, padding: "18px 24px", color: "#fff" }}>
                  ...
                  <div style={{ fontSize: 12, opacity: .75 }}>Dir. {dir} · {mesLabel} · Día hábil {diaActual}/{diasHabiles}</div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 14 }}>
                    {[{l:"Meta mes",v:fmtMXN(s.metaMonto)},{l:"Acumulado",v:fmtMXN(acu)},{l:"Esperado",v:fmtMXN(esperadoHoy(s.metaMonto))}].map(item => (
                      <div key={item.l} style={{ textAlign: "center" }}>
                        <div style={{ fontSize: 10, opacity: .7, fontWeight: 600, textTransform: "uppercase" }}>{item.l}</div>
                ...
                <div style={{ background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 1px 6px rgba(0,0,0,0.07)", border: "1px solid #e2e8f0" }}>
                  ...
                  <div style={{ padding: 20 }}>
                    {capturaHoy ? (
                      editDia?.dia === diaActual ? (
                        <div>
                          <input type="number" min="0" value={editDia.monto} onChange={e => setEditDia(d => ({ ...d, monto: e.target.value }))}
                            style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: `1.5px solid ${col}`, fontSize: 17, fontFamily: "'DM Mono'", outline: "none", boxSizing: "border-box" }} />
                          <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                            <button onClick={() => setEditDia(null)} style={{ flex: 1, padding: "10px", borderRadius: 10, border: "1.5px solid #e2e8f0", background: "#fff", color: "#64748b", fontFamily: "inherit", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>Cancelar</button>
                            <button onClick={async () => { await guardarEnSheets(dir, suc, diaActual, editDia.monto); setEditDia(null); }} disabled={guardando}
                ...
                <button onClick={() => { setVista("detalle"); setEditando(null); setEditDia(null); }}
